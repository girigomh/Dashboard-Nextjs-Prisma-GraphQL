...
export default function BankAccountSection() {
  ...
  const hasBankDetails = watch('hasBankDetails') as boolean;

  const [canEdit, setCanEdit] = useState(!hasBankDetails);
  const { t } = useTranslation('users');
  const toggleCanEdit = useCallback(() => {
    setCanEdit((value) => !value);
    setValue('hasBankDetails', false);
  }, [setCanEdit, setValue]);
    ...
    <FormSection>
      ...
      {hasBankDetails && (
        <div className="text-end">
          ...
          >
            <i className="uil uil-edit" role="button" /> Edit bank details
          </button>
        </div>
      )}
    </FormSection>
