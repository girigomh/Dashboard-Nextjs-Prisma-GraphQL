...
apiRoute.get(async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    ...
    const blob = await prisma.attachmentBlob.findFirst({
      where: {
        filename: { equals: filename as string },
        attachment: { some: { recordType: 'Deduction', recordId: Number(id!) } }
      },
    ...
    await streamFileFromS3({ res, key: blob.key, contentType: blob.contentType! });
    const end = new Date().getTime();
    const duration = end - start;
    logger.info(`${req.url}: streamed deduction file from aws in ${duration}ms`);
  } catch (err) {
    logger.error(err);
